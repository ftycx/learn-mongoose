...
async function getUser() {
    try {
        ...
        td.textContext = user.name; // 사용자 이름을 셀에 추가
        row.appendChild(td);
        td = document.createElement('td');
        td.textContent = user.age; // 사용자 이름을 셀에 추가
        row.appendChild(td);  
        td = document.createElement('td');
...
async function getComment(id) {
    try {
        ...
        td = document.createElement('td');
        td.textContent = comment.comment.name;
        row.appendChild(td);
        // 댓글 내용을 표시하는 셀을 생성하고 콘텐츠를 추가
        ...
        edit.addEventListener('click', async () => {
            ...
            const newComment = prompt('바꿀 내요을 입력하세요'); // 사용자에게 새로운 댓글 내용을 입력받음
            if (!newComment) {
            return alert('내용을 반드시 입력하셔야 합니다'); // 새로운 댓글 내용이 없는 경우 경고를 포시하고 함수를 종료
            }
            try {
...
document.getElementById('user-form').addEventListener('submit', async (e) => {
    ...
    const marride = e.target.married.checked; // 결혼 여부를 가져옵니다.
    if (!name) {
        return alert('이름을 입력하세요'); // 이름이 입력되지 않은 경우 경고를 표시하고 함수 종료
    }
    if (!age) {
        return alert('나이를 입력하세요'); // 나이가 입력되지 않은 경우 경고를 표시하고 함수 종룍
    }
    try {
